...
func defaultCascadeSpectrumConfigs(cfg SpectrumConfig) []cascadeSpectrumConfig {
	...
	return []cascadeSpectrumConfig{
		{
			// Long support only. A larger domain prevents obvious repetition; low weight
			// prevents the broad height/color blobs from returning.
			DomainSize:     1280.0,
			PeakWavelength: 66.0,
			MinWavelength:  34.0,
			MaxWavelength:  150.0,
		...
		{
			// Main wind-sea geometry. This remains the real displacement layer, but with
			// a larger non-obvious tile and less perfectly repeated short structure.
			DomainSize:     360.0,
			PeakWavelength: 11.5,
			MinWavelength:  4.2,
			MaxWavelength:  30.0,
		...
		{
			// Short chop with a much larger domain. The old 38m domain repeated visibly
			// in top-down views. Larger domain + normal filtering keeps detail but hides
			// the FFT tile.
			DomainSize:     128.0,
			PeakWavelength: 3.35,
			MinWavelength:  1.35,
		...
		{
			// Capillary/slope domain. No meaningful displacement; large enough that a
			// camera above the surface does not see a repeating stamp.
			DomainSize:     64.0,
			PeakWavelength: 1.35,
			MinWavelength:  0.72,
			MaxWavelength:  3.2,
